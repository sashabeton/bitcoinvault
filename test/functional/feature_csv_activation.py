...
def create_bip112special(node, input, txversion, address):
    tx = create_transaction(node, input, address, amount=Decimal("349.98"))
...
def send_generic_input_tx(node, coinbases, address):
    return node.sendrawtransaction(ToHex(sign_transaction(node, create_transaction(node, node.getblock(coinbases.pop())['tx'][0], address, amount=Decimal("349.99")))))
...
def create_bip68txs(node, bip68inputs, txversion, address, locktime_delta=0):
    ...
    for i, (sdf, srhb, stf, srlb) in enumerate(product(*[[True, False]] * 4)):
        ...
        tx = create_transaction(node, bip68inputs[i], address, amount=Decimal("349.98"))
        tx.nVersion = txversion
...
def create_bip112txs(node, bip112inputs, varyOP_CSV, txversion, address, locktime_delta=0):
    ...
    for i, (sdf, srhb, stf, srlb) in enumerate(product(*[[True, False]] * 4)):
        locktime = relative_locktime(sdf, srhb, stf, srlb)
        tx = create_transaction(node, bip112inputs[i], address, amount=Decimal("349.98"))
...
class BIP68_112_113Test(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        bip113tx_v1 = create_transaction(self.nodes[0], bip113input, self.nodeaddress, amount=Decimal("349.98"))
        bip113tx_v1.vin[0].nSequence = 0xFFFFFFFE
        bip113tx_v1.nVersion = 1
        bip113tx_v2 = create_transaction(self.nodes[0], bip113input, self.nodeaddress, amount=Decimal("349.98"))
