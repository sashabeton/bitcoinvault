...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(self.nodes[0].getbalance(), 350)
        assert_equal(self.nodes[1].getbalance(), 350)

        self.log.info("Test getbalance with different arguments")
        assert_equal(self.nodes[0].getbalance("*"), 350)
        assert_equal(self.nodes[0].getbalance("*", 1), 350)
        assert_equal(self.nodes[0].getbalance("*", 1, True), 350)
        assert_equal(self.nodes[0].getbalance(minconf=1), 350)

        # Send 40 BTCR from 0 to 1 and 60 BTCR from 1 to 0.
        txs = create_transactions(self.nodes[0], self.nodes[1].getnewaddress(), 40, [Decimal('0.01')])
        self.nodes[0].sendrawtransaction(txs[0]['hex'])
        self.nodes[1].sendrawtransaction(txs[0]['hex'])  # sending on both nodes is faster than waiting for propagation

        self.sync_all()
        txs = create_transactions(self.nodes[1], self.nodes[0].getnewaddress(), 360, [Decimal('0.01'), Decimal('0.02')])
        ...
        assert_equal(self.nodes[0].getbalance(), Decimal('309.99'))  # change from node 0's send
        assert_equal(self.nodes[1].getbalance(), Decimal('29.99'))  # change from node 1's send
        # Same with minconf=0
        assert_equal(self.nodes[0].getbalance(minconf=0), Decimal('309.99'))
        ...
        assert_equal(self.nodes[0].getunconfirmedbalance(), Decimal('360'))  # output of node 1's spend
        ...
        assert_equal(self.nodes[0].getwalletinfo()["unconfirmed_balance"], Decimal('360'))  # output of node 1's send
        assert_equal(self.nodes[0].getunconfirmedbalance(), Decimal('360'))
        ...
        assert_equal(self.nodes[0].getbalance(), Decimal('669.99'))  # node 1's send plus change from node 0's send
