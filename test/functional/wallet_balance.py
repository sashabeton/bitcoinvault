...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(self.nodes[1].getbalance(), 175)

        self.log.info("Test getbalance with different arguments")
        assert_equal(self.nodes[0].getbalance("*"), 175)
        assert_equal(self.nodes[0].getbalance("*", 1), 175)
        assert_equal(self.nodes[0].getbalance("*", 1, True), 175)
        assert_equal(self.nodes[0].getbalance(minconf=1), 175)
        ...
        self.sync_all()
        txs = create_transactions(self.nodes[1], self.nodes[0].getnewaddress(), 185, [Decimal('0.01'), Decimal('0.02')])
        ...
        assert_equal(self.nodes[0].getbalance(), Decimal('134.99'))  # change from node 0's send
        assert_equal(self.nodes[1].getbalance(), Decimal('29.99'))  # change from node 1's send
        # Same with minconf=0
        assert_equal(self.nodes[0].getbalance(minconf=0), Decimal('134.99'))
        ...
        assert_equal(self.nodes[0].getunconfirmedbalance(), Decimal('185'))  # output of node 1's spend
        ...
        assert_equal(self.nodes[0].getwalletinfo()["unconfirmed_balance"], Decimal('185'))  # output of node 1's send
        assert_equal(self.nodes[0].getunconfirmedbalance(), Decimal('185'))
        ...
        assert_equal(self.nodes[0].getbalance(), Decimal('319.99'))  # node 1's send plus change from node 0's send
