...
# Objects that map to bvaultd objects, which can be serialized/deserialized
...
class CTransaction:
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in bvaultd
            if (flags != 0):
                self.vin = deser_vector(f, CTxIn)
...
class CAuxPow(CTransaction):
    __slots__ = ("hashBlock", "vMerkleBranch", "nIndex", "vChainMerkleBranch", "nChainIndex", "parentBlock")
...
class CBlock(CBlockHeader):
    ...
    def deserialize(self, f):
        ...
        self.vatx = deser_vector(f, CTransaction)
    ...
    def serialize(self, with_witness=False):
        ...
        if with_witness:
            ...
            r += ser_vector(self.vatx, "serialize_with_witness")
        else:
            r += ser_vector(self.vtx, "serialize_without_witness")
            r += ser_vector(self.vatx, "serialize_without_witness")
        return r
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in bvaultd indicates these should be deserialized as blocks
