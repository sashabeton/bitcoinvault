...
class AlertsInstantTest(BitcoinTestFramework):
    ...
    def test_recovery_tx_is_rejected_when_missing_recovery_key(self):
        alert_addr0 = self.nodes[0].getnewvaultinstantaddress(self.alert_instant_pubkey, self.alert_recovery_pubkey)
        ...
        error = None
        try:
            self.nodes[0].signrecoverytransaction(recoverytx, [self.alert_instant_privkey], alert_addr0['redeemScript'])
    ...
    def test_recovery_tx_is_rejected_when_missing_instant_key(self):
        ...
        error = None
        try:
            self.nodes[0].signrecoverytransaction(recoverytx, [self.alert_recovery_privkey], alert_addr0['redeemScript'])
        except Exception as e:
            error = e.error
    ...
    def test_recovery_tx_when_both_instant_and_recovery_keys_given(self):
        ...
        # recover atx
        recoverytx = self.nodes[0].createrecoverytransaction(atxid, [{addr0: 174.99}])
        self.nodes[0].signrecoverytransaction(recoverytx, [self.alert_instant_privkey, self.alert_recovery_privkey], alert_addr0['redeemScript'])

        # assert
        assert recoverytx is not None
        assert recoverytx != ''

    # TODO-fork update those tests to use alertinstant address
