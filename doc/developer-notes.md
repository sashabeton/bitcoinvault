...
([`valgrind.supp`](https://github.com/bitcoinroyale/bitcoinroyale/blob/master/contrib/valgrind.supp))
...
$ valgrind -v --leak-check=full src/broyaled -printtoconsole
```
...
To profile a running broyaled process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep broyaled` -- sleep 60
```
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `broyale-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `broyale-cli` and the GUI debug console use this table to determine how to
