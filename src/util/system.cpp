...
const char * const BITCOIN_CONF_FILENAME = "bvault.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bvaultd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitcoinVault
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitcoinVault
    // Mac: ~/Library/Application Support/BitcoinVault
    // Unix: ~/.bvault
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinVault";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitcoinVault";
#else
    // Unix
    return pathRet / ".bvault";
#endif
#endif
}
