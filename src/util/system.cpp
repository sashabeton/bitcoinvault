...
const char * const BITCOIN_CONF_FILENAME = "broyale.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "broyaled -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitcoinRoyale
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitcoinRoyale
    // Mac: ~/Library/Application Support/BitcoinRoyale
    // Unix: ~/.broyale
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinRoyale";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitcoinRoyale";
#else
    // Unix
    return pathRet / ".broyale";
#endif
#endif
}
